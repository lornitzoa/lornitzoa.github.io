$(() => {
  ...
  $('.mealItems').droppable({ // this codes works to drag and drop once to any mealITem field
    ...
    drop: (e, ui) => {
      let $dropTarget = $(e.target); // new meal
      let $dragItem = $('.ui-draggable-dragging'); // item being dragged
      $($dragItem).appendTo($dropTarget).css({top:0});
      console.log('new parent: ' + $($dropTarget).parents().attr('class'));
      console.log('dropped item: ' + $dragItem);
      console.log('original meal: ' + meal); // original parent
      console.log($dragItem);
      console.log($($dragItem).parent()); // new parent
      saveToLocalStorage($dragItem)
      removeFromLocalStorage(draggedItem, meal);
      // deleteItems()
      calcMealTotals()
      // console.log(meal);
      // updateLocalStorage()
  ...
  const addItemToMeal = (e) => {
    ...
    $($clone).on('dragstart', (e) => {
      meal = $(e.target).parents().eq(1).attr('class')
  ...
  const retrieveLocalStorage = () => {
    ...
    let itemDetailsArr = ['item', 'totalCal','carbs','protein','fat','fiber','sugar']
